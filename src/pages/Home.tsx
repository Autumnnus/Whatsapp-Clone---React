...
const Home = () => {
  ...
  return (
    <div className="flex w-full h-full bg-whatsapp-bg overflow-hidden relative">
      {/* Left Sidebar */}
      <div
        className={`flex flex-col h-full bg-whatsapp-panel border-r border-whatsapp-border transition-all duration-300 relative z-10 ${token ? "w-full md:w-[35%] lg:w-[30%] min-w-[300px] max-w-[450px]" : "w-full md:w-[30%]"}`}
      >
        <LeftHeader
        ...
        {token && (
          <div className="px-2 py-2 bg-whatsapp-panel">
            <SearchInput />
          </div>
        )}
      ...
      {/* Right Chat Area */}
      <div
        className={`flex-col flex-1 h-full bg-whatsapp-dark relative ${token ? "hidden md:flex" : "flex w-full"}`}
      >
        {!loginMode && !token ? (
          ...
          <div className="flex flex-col h-full w-full relative">
            ...
            <div className="flex-1 overflow-hidden relative z-1 p-0">
